    ...
    module GoogleDrive
      ...
      def flush_writes
        @queued_for_write.each do |style, file|
          if exists?(path(style))
            fail FileExists, "file \"#{path(style)}\" already exists in your Google Drive"
          else
            ...
            title = title_for_file(style)
            mime_type = "#{content_type}"
            parent_id = @google_drive_options[:public_folder_id] # folder_id for Public folder
            metadata = drive.files.insert.request_schema.new('title' => title, # if it is no extension, that is a folder and another folder
                                                             'description' => 'paperclip file on google drive',
                                                             'mimeType' => mime_type)
            metadata.parents = [{ 'id' => parent_id }] if parent_id
      ...
      def find_public_folder
        unless @google_drive_options[:public_folder_id]
          fail KeyError, 'you must set a Public folder if into options'
        end
      ...
      def parse_credentials(credentials)
        result =
          ...
          when Hash
            ...
            fail ArgumentError, ':google_drive_credentials are not a path, file, nor a hash'
          end
